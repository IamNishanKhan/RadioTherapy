...
def export_original_images_nifti():
    ...
    # Extract patient ID from DICOM database or user input
    ...
    original_nifti_path = os.path.join(output_dir, f"{patient_id}_OriginalImages3D_Nifty.nii.gz")
...
def export_to_nifti_and_png():
    ...
    output_dir = os.path.join(base_output_dir, f"{patient_id}_ExtractedData")
    os.makedirs(output_dir, exist_ok=True)
    mask_nifti_path = os.path.join(output_dir, f"{patient_id}_Labeled3D_Nifty.nii.gz")

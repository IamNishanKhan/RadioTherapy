...
def export_base_images(volume_node, output_dir, patient_id):
    ...
    for z in range(image_data.shape[2]):
        ...
        base_image.save(os.path.join(coronal_dir, f'{patient_id}_axial_base_{z:03d}.png'))
        print(f"Saved axial base image {os.path.join(coronal_dir, f'{patient_id}_axial_base_{z:03d}.png')}")
    ...
    for x in range(axial_img.shape[0]):
        ...
        base_image = base_image.resize((512, 512), Image.NEAREST)
        base_image.save(os.path.join(axial_dir, f'{patient_id}_coronal_base_{x:03d}.png'))
        print(f"Saved coronal base image {os.path.join(axial_dir, f'{patient_id}_coronal_base_{x:03d}.png')}")

...
def nifti_to_png(nifti_path, output_dir):
    ...
    for z in range(img.shape[2]):
        ...
        # Convert to PIL Image, resize, and rotate 90 degrees to the left
        colored_image = Image.fromarray(colored_slice)
        colored_image = colored_image.resize((512, 512), Image.NEAREST)
        colored_image = colored_image.rotate(90)  # Rotate 90 degrees counterclockwise
    ...
    # Coronal view (as per your definition) shows the y-z plane (back-front, bottom-top) at each x level
    ...
    # Sagittal view (as per your definition) shows the x-z plane (left-right, bottom-top) at each y level
    ...
    # --- Count unique color classes across all PNGs ---
    print("\nCounting unique color classes across all PNGs...")
    unique_colors = set()
    for subdir in ["axial", "coronal", "sagittal"]:
        dir_path = os.path.join(output_dir, "pngs", subdir)
    ...
    print(f"Total number of unique color classes: {len(unique_colors)}")
    print(f"Unique colors (RGB tuples): {sorted(unique_colors)}")

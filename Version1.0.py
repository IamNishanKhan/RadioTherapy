...
def nifti_to_png(nifti_path, output_dir):
    ...
    # --- Count unique color classes across all mask PNGs ---
...
def export_to_nifti_and_png():
    ...
    mask_nifti_path = os.path.join(output_dir, f"{sanitized_name}_refied_masks_only.nii.gz")
    
    # Step 5: Export original NIfTI with only included structures
    original_labelmap_volume_node = export_original_nifti(segmentation_node, original_nifti_path)
    
    # Step 6: Export base images from DICOM volume for Axial, Coronal, and Sagittal views
    export_base_images(volume_node, output_dir)
    
    # Step 7: Export mask NIfTI with custom labels
    mask_labelmap_volume_node = export_segmentation_as_nifti(segmentation_node, mask_nifti_path)
    
    # Step 8: Convert mask NIfTI to mask PNGs for Axial, Coronal, and Sagittal views
    nifti_to_png(mask_nifti_path, output_dir)
    
    # Step 9: Convert mask NIfTI to contour PNGs for Axial, Coronal, and Sagittal views
